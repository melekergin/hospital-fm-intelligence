...
SELECT
    ...
    year,
    year_sort,
    maintenance_backlog_cost_gbp                                                  AS current_backlog_gbp,
    backlog_cost_per_bed                                                          AS current_backlog_per_bed,

    LAG(maintenance_backlog_cost_gbp) OVER (PARTITION BY trust_code ORDER BY year_sort)
    ...
    ROUND(
        (maintenance_backlog_cost_gbp
            - LAG(maintenance_backlog_cost_gbp) OVER (PARTITION BY trust_code ORDER BY year_sort))
        / NULLIF(LAG(maintenance_backlog_cost_gbp) OVER (PARTITION BY trust_code ORDER BY year_sort), 0) * 100,
